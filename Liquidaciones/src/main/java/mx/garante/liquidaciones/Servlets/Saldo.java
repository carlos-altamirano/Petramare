                ...
                if (usuarioApp != null) {
                    try {
                        ...
                        if (fecha_ini != null && !fecha_ini.equals("")) {
                            String tmpo = ModeloLayOut.compruebaFechaOperacion(fecha_ini);
                            if (!tmpo.equals("")) {
                                mensaje = new Message();
                                mensaje.setDesc(tmpo);
                                session.setAttribute("messageBean", mensaje);
                                fecha_ini = ModeloLayOut.obtenFormatoFecha(fecha_ini, 0);
                            }
                        } else {
                            fecha_ini = ModeloLayOut.obtenFormatoFecha(fecha_ini, 0);
                        }
                        ...
                        if (fecha_fin != null && !fecha_fin.equals("")) {
                            String tmpo = ModeloLayOut.compruebaFechaOperacion(fecha_fin);
                            if (!tmpo.equals("")) {
                                mensaje = new Message();
                                mensaje.setDesc(tmpo);
                                session.setAttribute("messageBean", mensaje);
                                fecha_fin = ModeloLayOut.obtenFormatoFecha(fecha_fin, 1);
                            }
                        } else {
                            fecha_fin = ModeloLayOut.obtenFormatoFecha(fecha_fin, 1);
                        }
                        ...
                        movimientos = ModeloLayOut.getMovimientos(clave_contrato, fecha_ini, fecha_fin, tipo_operacion);
//                nombresEdoCta = Modelos.ModeloLayOut.get10DatesEdoCta(clave_contrato);
                        nombresEdoCta = ModeloLayOut.getEdoCtaFiles(clave_contrato);
...
//              movimientos = Modelos.ModeloLayOut.getMovimientos("", "", "");
                        ...
                        movimientos = ModeloLayOut.getLiduiacionesPendietes(clave_contrato, saldo_actual);
                        Vector saldos = null;
