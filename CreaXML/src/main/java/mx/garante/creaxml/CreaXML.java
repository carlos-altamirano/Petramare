...
//    private static final String BASE_PATH = "c:" + File.separator +  "inetpub" + File.separator + "ftproot" + File.separator;
    ...
    private static void menu() {
        ...
        while (res) {
            ...
            System.out.println("0 .- Proceso fin de mes sin timbrado");
            ...
            switch (entradaTeclado) {
	            case "0": {  
                    ...
                    generaEdoCuentaSinTimbrar(fechaHoy);
                    generaNominaSinTimbrar(fechaHoy);
                    break;
	            }        
                case "1": {
                    System.out.print("Ingresa la fecha en este formato yyyy-MM en caso contrario dejar vacio -> ");
    ...
    private static void generaNominaSinTimbrar(Date fechaHoy) {
...
//                marshaller.setProperty("com.sun.xml.internal.bind.namespacePrefixMapper", SPACE_MAPPER);
                ...
                if (compNomina.getUuid() == null) {
                    ...
                    compNomina.setFechaTimbre(fechaHoy);
                    compNomina.setUuid("1");
                    // insertar en BD
				...
				if (totalTimbrados == rfcs.size()) {
					...
					String salida = BASE_PATH + File.separator + "CFDI" + File.separator + rfc + File.separator
							+ c2.get(Calendar.YEAR) + File.separator;
					...
					CreaPDF.nomina(comprobante, timbre, receptor, emisor, emisorNom, receptorNom, nomina, movimientos,
							concepto1, listOtroPagos, salida + meses[c2.get(Calendar.MONTH)], cadorigen,
							contrato.getNombre_cliente());

					File files[] = new File[2];
					files[0] = new File(salida + meses[c2.get(Calendar.MONTH)] + ".xml");
					files[1] = new File(salida + meses[c2.get(Calendar.MONTH)] + ".pdf");

					File zip = new File(BASE_PATH + File.separator + "CFDI" + File.separator + receptor.getRfc()
							+ File.separator + fecha.substring(0, 4) + File.separator
							+ Fecha.nombreMes(fecha.substring(4, 6)) + ".zip");
					try {
    ...
    private static void generaEdoCuentaSinTimbrar(Date fechaHoy) {
...
//                marshaller.setProperty("com.sun.xml.internal.bind.namespacePrefixMapper", SPACE_MAPPER);
                ...
                if (compEdoCta.getUuid() == null) {
                    ...
                    compEdoCta.setFechaTimbre(fechaHoy);
                    compEdoCta.setUuid("1");
...
//                    compEdoCta.setSelloSAT(timbre33.getSelloSAT().getValue());
				...
				if (totalTimbrados == contratos.size()) {
					...
					tdf.setFechaTimbrado(Fecha.crearXMLGregorianCalendar(new Date()));
//                    tdf.setFechaTimbrado(Fecha.crearXMLGregorianCalendar(compEdoCta.getFechaTimbre()));
					...
					edoDeCuenta.setAportacionTotalFideicomiso(
							new BigDecimal(totalAbono).setScale(2, RoundingMode.HALF_UP));
					edoDeCuenta.setContrato(contrato.getClave_contrato());
					String fechaEdoCta = format.format(c1.getTime()).substring(8, 10) + " al "
							+ format.format(c2.getTime()).substring(8, 10) + " de " + meses[c1.get(Calendar.MONTH)]
							+ " de " + c1.get(Calendar.YEAR);
					edoDeCuenta.setFechaEdoCta(fechaEdoCta);
					edoDeCuenta
							.setHonorariosFiduciarios(new BigDecimal(totalHonorario).setScale(2, RoundingMode.HALF_UP));
					edoDeCuenta.setIvaHonorariosFiduciarios(new BigDecimal(totalIVA).setScale(2, RoundingMode.HALF_UP));
					edoDeCuenta.setLiquidacionFideicomisarios(
							new BigDecimal(totalOrdenLiq).setScale(2, RoundingMode.HALF_UP));
					edoDeCuenta.setMoneda(CMoneda.MXN.toString());
					edoDeCuenta.setRestitucionPatrimonio(
							new BigDecimal(totalRestitucion).setScale(2, RoundingMode.HALF_UP));
					edoDeCuenta.setRfcFideicomitente(contrato.getRFC());
					edoDeCuenta.setSaldoAnteriorPatrimonio(
							new BigDecimal(saldoAnterior).setScale(2, RoundingMode.HALF_UP));
					...
					String salida = BASE_PATH + "EstadosDeCuenta" + File.separator + contrato.getClave_contrato()
							+ File.separator + c2.get(Calendar.YEAR) + File.separator;
					File carpetas = new File(salida);
					carpetas.mkdirs();
					...
					CreaPDF.edoCuenta(contrato, saldoAnterior, saldoFinal, totalRestitucion, totalOrdenLiq,
							totalHonorario, totalIVA, totalAbono, fechaEdoCta, tdf,
							salida + meses[c2.get(Calendar.MONTH)], cadorigen, emisor.getRfc());

					File files[] = new File[2];
					files[0] = new File(salida + meses[c2.get(Calendar.MONTH)] + ".xml");
					files[1] = new File(salida + meses[c2.get(Calendar.MONTH)] + ".pdf");
					File zip = new File(BASE_PATH + File.separator + "EstadosDeCuenta" + File.separator
							+ contrato.getClave_contrato() + File.separator + "EDOCTA_" + contrato.getClave_contrato()
							+ "_" + c2.get(Calendar.YEAR) + "_" + Empaquetar.numMes(c2.get(Calendar.MONTH)) + ""
							+ ".zip");
...
//                marshaller.setProperty("com.sun.xml.internal.bind.namespacePrefixMapper", SPACE_MAPPER);
                ...
                if (totalTimbrados == contratos.size()) {
                    ...
                    String salida = BASE_PATH + File.separator + "EstadosDeCuenta" + File.separator + contrato.getClave_contrato() + File.separator + c2.get(Calendar.YEAR) + File.separator;
                    File carpetas = new File(salida);
                    carpetas.mkdirs();
                    marshaller.setProperty(Marshaller.JAXB_SCHEMA_LOCATION, LOCATIONFinal);
                    marshaller.marshal(comprobante, new File(salida + meses[c2.get(Calendar.MONTH)] + ".xml"));
                    ...
                    File zip = new File(BASE_PATH + File.separator + "EstadosDeCuenta" + File.separator + contrato.getClave_contrato() + File.separator + "EDOCTA_" + contrato.getClave_contrato() + "_" + c2.get(Calendar.YEAR) + "_" + Empaquetar.numMes(c2.get(Calendar.MONTH)) + "" + ".zip");
...
//                nomina.setFechaFinalPago(formatNomina.format(c2.getTime()));
                ...
                if (totalTimbrados == rfcs.size()) {
                    ...
                    String fecha = format2.format(fechaHoy);
                    String salida = BASE_PATH + File.separator + "CFDI" + File.separator + rfc + File.separator + c2.get(Calendar.YEAR) + File.separator;
                    File carpetas = new File(salida);
                    carpetas.mkdirs();
                    ...
                    File zip = new File(BASE_PATH + File.separator + "CFDI" + File.separator + receptor.getRfc() + File.separator + fecha.substring(0, 4) + File.separator + Fecha.nombreMes(fecha.substring(4, 6)) + ".zip");
                    try {
                        Empaquetar.addFilesToExistingZip2(zip, files);
                        if (!Empaquetar.eliminaR(files[0])) {
                            System.out.println("Error al eliminar " + carpetas.getAbsolutePath());
